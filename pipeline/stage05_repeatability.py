...
if __name__ == "__main__":
    ...
    # 1. compute repeatability scores
    rep = compute_repeatability(df)
    rep_out = cfg["combined_out"].replace("combined_metrics.csv", "repeatability_scores.csv")
    rep.to_csv(rep_out, index=False)
    print(f"  Repeatability scores → {rep_out}")

    # 2. confirm executable repeatability for top offenders
    data_root = Path(cfg["darshan_data"])
    exe_df = confirm_executable_repeatability(rep, df, data_root, top_n=5)
    if exe_df is not None:
        exe_out = cfg["combined_out"].replace("combined_metrics.csv", "executable_repeatability.csv")
        exe_df.to_csv(exe_out, index=False)
        print(f"  Executable repeatability → {exe_out}")
    djc_rep = confirm_djc_repeatability(rep, df, top_n=10)
    djc_out = cfg["combined_out"].replace("combined_metrics.csv", "djc_repeatability.csv")
    djc_rep.to_csv(djc_out, index=False)
    print(f"  DJC repeatability → {djc_out}")
